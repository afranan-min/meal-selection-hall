...
import '../styles/MealRoutinePage.css';
const days = ['Saturday', 'Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'];
const meals = ['Morning', 'Noon', 'Dinner'];

const MealRoutinePage = () => {
  const [routine, setRoutine] = useState([]);

  useEffect(() => {
    const fetchRoutine = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/admin/get-routine');
...
export default MealRoutinePage;
